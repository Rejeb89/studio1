...
const ExtractTextFromImageInputSchema = z.object({
  imageDataUri: z
    .string()
    .describe(
      "An image with text, as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'."
    ),
});
...
const extractTextFromImagePrompt = ai.definePrompt({
  ...
  prompt: `You are an OCR (Optical Character Recognition) engine. Your task is to extract any and all text from the provided image.
- If there is text in the image, return it exactly as it appears.
